...
class ImgAugment(object):
    def __init__(self, width = 512, height = 512, phase='train'):
        ...
        self.transform_train = iaa.Sequential(
            [
                iaa.HorizontalFlip(),
                # iaa.size.CenterCropToFixedSize(width=self.width+32, height=self.height+32)
            ]
        )
    ...
    def __call__(self, image, bbs, kps=None):
...
class WheetDataset(Dataset):
    def __init__(self, df, root='./data', phase='train', transforms = None):
        super(Dataset, self).__init__()
        self.df = df
        self.image_ids = df['image_id'].unique()
        self.root = root
        self.phase = phase
        self.transforms = ImgAugment(phase=phase)
    def __getitem__(self, idx):
        img_path = os.path.join(self.root, self.phase, self.image_ids[idx]+'.jpg')
